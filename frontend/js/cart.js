...
const showCart = (productCart) => {
    ...
    for (let i = 0; i < productCart.articles.length; i++) {
        ...
        // dibuja la papelera DESAFIATE 7 
    ...
    deleteElement(productCart);// Desafiate 7
    ...
    for (let i = 0; i < cantProduct.length; i++) {
        cantProduct[i].addEventListener("keyup", () => {
            ...
            for (let x = 0; x < cantProduct.length; x++) {
                ...
                //     alert("La cantidad de productos debe ser mayor a 0 para efectuar la compra");
                // }
            }
...
function controlCantCero() {
    ...
    for (let i = 0; i < cantProduct.length; i++) {
        ...
        let cant = parseInt(document.getElementById('Cant_' + i).value);
        if (cant == 0) { 
            okCantidad = false; 
        }
...
document.addEventListener("DOMContentLoaded", async function (e) {
    ...
    document.getElementById("irAPago").addEventListener("click", () => {

        let okCantProducts = true;

        okCantProducts = controlCantCero();

        if (okCantProducts == false) {
